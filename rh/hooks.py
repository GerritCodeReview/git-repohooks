    ...
    for arg in args:
      if arg.endswith("${PREUPLOAD_FILES_PREFIXED}"):
        if arg == "${PREUPLOAD_FILES_PREFIXED}":
          ...
          ret.extend(prefix + file for file in self.get("PREUPLOAD_FILES"))
      else:
        ...
        for key, val in replacements.items():
          ...
          ret.append(
              re.sub(r"\$\{(" + "|".join(all_vars) + r")\}", replace, arg)
          )
    return ret
...
def check_google_java_format(project, commit, _desc, _diff, options=None):
  ...
  patterns = [rf"^{x}/.*\.java$" for dir_list in include_dirs for x in dir_list]
  if not patterns:
    patterns = [r"\.java$"]
...
def check_commit_msg_changeid_field(project, commit, desc, _diff, options=None):
  ...
  if not found:
    ...
    error = (
        f'Commit message has too many "{field}:" lines.  There can be only one.'
    )
  else:
    return None
