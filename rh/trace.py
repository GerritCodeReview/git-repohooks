...
def _write_trace2_event(event_dict: Dict[str, Any]) -> None:
    ...
    try:
        if target.startswith("af_unix:"):
            ...
            if sock_path.startswith("af_unix:dgram:"):
                sock_path = sock_path[len("af_unix:dgram:") :]
                sock_type = socket.SOCK_DGRAM
            elif sock_path.startswith("af_unix:stream:"):
                sock_path = sock_path[len("af_unix:stream:") :]
            elif sock_path.startswith("af_unix:"):
                sock_path = sock_path[len("af_unix:") :]
