...
class PlaceholderTests(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.replacer = rh.hooks.Placeholders(
        [rh.git.RawDiffEntry(file=x) for x in ["path1/file1", "path2/file2"]]
    )
  ...
  @mock.patch.object(rh.git, "find_repo_root", side_effect=mock_find_repo_root)
...
class BuiltinHooksTests(unittest.TestCase):
  ...
  def test_alint(self, mock_check, mock_run):
    ...
    commitOne = "Add test to the manifest Bug: 11111 Test: ... Flag: ..."
    diff = [rh.git.RawDiffEntry(file="file.txt", status="A")]
    ret = rh.hooks.check_alint(
        self.project, commitOne, "desc", diff, options=self.options
    )
    self.assertIsNotNone(ret)
    self.assertFalse(mock_check.called)
    ...
    diff = []
    ret = rh.hooks.check_alint(
        self.project, commitTwo, "desc", diff, options=self.options
    )
    self.assertIsNone(ret)
