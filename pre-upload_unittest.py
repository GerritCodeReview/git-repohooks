...
class PreUploadMainTests(unittest.TestCase):
    ...
    def test_main_fix_propagation(self, mock_run):
        ...
        mock_run.assert_called_once_with(["project"], [None], fix=True, yes=True)
    ...
    @mock.patch("rh.utils.run")
    def test_attempt_fixes_with_fix_flag(self, mock_run, mock_prompt, _mock_isatty):
        """Verify _attempt_fixes with fix=True executes fixups without prompting."""
        hook_result = rh.results.HookResult(
            "test_hook", "test_project", "test_commit", error="error", fixup_cmd=["fix_cmd"]
        )
        proj_result = rh.results.ProjectResults("test_project", "/workdir", [hook_result])
