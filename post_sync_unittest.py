...
class PostSyncTests(unittest.TestCase):
    ...
    @mock.patch("rh.git.find_repo_root")
    @mock.patch("os.path.exists")
    @mock.patch("builtins.open", new_callable=mock.mock_open, read_data="hooks/test-hook.py\n")
    @mock.patch("subprocess.run")
    def test_hook_execution(self, mock_run, _mock_open, mock_exists, mock_find_root):
        """Check successful hook execution."""
        mock_find_root.return_value = str(self.topdir)
        # We need to mock exists for both config_file and hook_path
        mock_exists.side_effect = lambda p: str(p) in (str(self.config_file), 
    ...
    @mock.patch("rh.git.find_repo_root")
    @mock.patch("os.path.exists")
    @mock.patch("builtins.open", new_callable=mock.mock_open, read_data="hooks/fail-hook.py\n")
    @mock.patch("subprocess.run")
    def test_hook_failure(self, mock_run, _mock_open, mock_exists, mock_find_root):
