...
def _run_post_sync_hooks(
    ...
    # Record the sync duration if available.
    if sync_duration_seconds is not None:
        history_file = repo_root_path / ".repo" / "post-sync-history.log"
        try:
            with open(history_file, "a", encoding="utf-8") as f:
                timestamp = datetime.datetime.now().isoformat()
                f.write(f"{timestamp} {sync_duration_seconds}\n")
        except OSError as e:
            print(
                f"Warning: Failed to record sync duration: {e}", file=sys.stderr
            )
    ...
    if sync_duration_seconds is not None:
        ...
        # Heuristic to distinguish incremental vs full sync (e.g., 10 mins).
        if sync_duration_seconds < 600:
            extra_env["REPO_SYNC_TYPE"] = "incremental"
        else:
            extra_env["REPO_SYNC_TYPE"] = "full"
...
def direct_main(argv):
    ...
    # Accept positional arguments to avoid crashing if repo passes them.
    parser.add_argument("projects", nargs="*", help="Ignored.")
