...
# We have to add the parent directory to sys.path so we can import rh.
...
def main(repo_topdir=None, **kwargs):
    ...
    env = os.environ.copy()
    env['REPO_TOPDIR'] = repo_topdir
    for key, value in kwargs.items():
        env[f'REPO_HOOK_{key.upper()}'] = str(value)
    ...
    with open(config_file, 'r') as f:
        for line in f:
            ...
            if not os.path.exists(hook_path):
                print(f"Warning: Registered post-sync hook not found: {hook_path}", file=sys.stderr)
                continue
            ...
            try:
                ...
                result = subprocess.run(cmd, cwd=repo_topdir, env=env, check=False)
                if result.returncode:
                    exit_code = result.returncode
            except Exception as e:
                print(f"Warning: Failed to execute post-sync hook '{line}': {e}", file=sys.stderr)
                exit_code = 1
