...
def main(repo_topdir=None, **kwargs):
    ...
    config_file = os.path.join(
        repo_topdir, ".repo", "manifests", "POSTSYNC.cfg"
    )
    ...
    with open(config_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not os.path.exists(hook_path):
                print(
                    "Warning: Registered post-sync hook not found: "
                    f"{hook_path}",
                    file=sys.stderr,
                )
                continue
...
if __name__ == "__main__":
    # TODO: Repo doesn't pass sync_duration_seconds yet.  Add support to
    # git-repo, then remove this hardcoded value.
