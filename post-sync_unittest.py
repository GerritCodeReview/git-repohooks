...
class PostSyncTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.topdir = Path(self.tempdir)
        self.manifest_dir = self.topdir / ".repo" / "manifests"
    ...
    def test_hook_execution(self, mock_run, mock_find_root):
        ...
        hook_path = self.topdir / hook_relative_path
        ...
        config.set("Hook Scripts", "test", hook_relative_path)
        ...
        self.assertEqual(self.topdir, kwargs["cwd"])
        self.assertEqual(
            str(self.topdir), kwargs["extra_env"]["REPO_TOPDIR"]
        )
