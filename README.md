...
They are dispatched via `post_sync.py` and allow projects to perform
...
This file is located in the manifest git repo at `.repo/manifests/POSTSYNC.cfg`.
It contains a list of paths to scripts (relative to the repo root) that should
be executed.

Example:
```
# Register post-sync hooks
vendor/google/tools/post-sync.py
```

### Hook Interface

Hooks are executed as standalone subprocesses. The following command-line flags
and environment variables are passed to each hook:

*   `--repo-topdir`: Absolute path to the root of the repo checkout.
*   `--sync-duration-seconds`: The total time taken by the sync operation.

Corresponding environment variables:
*   `REPO_TOPDIR`
*   `REPO_HOOK_SYNC_DURATION_SECONDS`

# Config Files (Pre-upload)
