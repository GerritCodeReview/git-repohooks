...
* `${REPO_SYNC_DURATION}`: The total time taken by the sync operation (post-sync hooks only).
* `${REPO_SYNC_TYPE}`: The type of sync operation executed (post-sync hooks only).
